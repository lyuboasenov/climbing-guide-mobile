...
namespace Alat.Validation.Impl {
   public class ValidationContext : Validation.ValidationContext, IDisposable {
      ...
      private Validatable Target { get; }
      private IDictionary<string, IEnumerable<string>> ValidationErrors { get; }
      private IDictionary<string, IEnumerable<Rule>> ValidationRules { get; }
      private IDictionary<string, PropertyInfo> ValueExtracters { get; }

      public ValidationContext(Validatable target, bool autoValidate = false) {
         Target = target ?? throw new ArgumentNullException(nameof(target));
         AutoValidate = autoValidate;

         ValidationErrors = new Dictionary<string, IEnumerable<string>>();
         ValidationRules = new Dictionary<string, IEnumerable<Rule>>();
      ...
      public void AddRule(string propertyName, params Rule[] rules) {
      ...
      public void AddRule<TSource, TProperty>(Expression<Func<TSource, TProperty>> propertyLambda, params Rule[] rules) 
      ...
      private void AddRule(PropertyInfo propertyInfo, params Rule[] rules) {
